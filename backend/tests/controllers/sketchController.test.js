...
describe("createSketch", () => {
  test("401 when unauthenticated", async () => {
    expect((await request(app).post("/sketches").send({})).status).toBe(401);
  });
  test("400 without a bookingId", async () => {
    const res = await request(app).post("/sketches").set("x-test-user-id", "a1").send({ imageUrls: ["u"] });
...
describe("listSketches", () => {
  test("401 when unauthenticated", async () => {
    expect((await request(app).get("/sketches")).status).toBe(401);
  });
  test("400 without a bookingId", async () => {
    expect((await request(app).get("/sketches").set("x-test-user-id", "c1")).status).toBe(400);
  });
  test("404 when the booking is missing", async () => {
    mockBooking.findById.mockResolvedValue(null);
    expect((await request(app).get("/sketches?bookingId=bk1").set("x-test-user-id", "c1")).status).toBe(404);
  });
  test("403 for a non-party", async () => {
    const res = await request(app).get("/sketches?bookingId=bk1").set("x-test-user-id", "stranger");
    expect(res.status).toBe(403);
  });
  test("returns the booking's sketches for a party", async () => {
    const res = await request(app).get("/sketches?bookingId=bk1").set("x-test-user-id", "c1");
    expect(res.body).toHaveLength(1);
  });
  test("500 on failure", async () => {
    mockBooking.findById.mockRejectedValue(new Error("db"));
    expect((await request(app).get("/sketches?bookingId=bk1").set("x-test-user-id", "c1")).status).toBe(500);
  });
});

describe("respondToSketch", () => {
  test("401 when unauthenticated", async () => {
    expect((await request(app).post("/sketches/sk1/respond").send({ action: "approve" })).status).toBe(401);
  });
  test("400 for an invalid action", async () => {
    const res = await request(app).post("/sketches/sk1/respond").set("x-test-user-id", "c1").send({ action: "nope" });
    expect(res.status).toBe(400);
  });
  test("404 when the sketch is missing", async () => {
    mockSketch.findById.mockResolvedValue(null);
    const res = await request(app).post("/sketches/sk1/respond").set("x-test-user-id", "c1").send({ action: "approve" });
    expect(res.status).toBe(404);
  });
  test("403 when a non-client responds", async () => {
    const res = await request(app).post("/sketches/sk1/respond").set("x-test-user-id", "a1").send({ action: "approve" });
    expect(res.status).toBe(403);
  });
