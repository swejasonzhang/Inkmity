...
describe("getAllMessagesForUser", () => {
  test("403 when reading another user's inbox", async () => {
    expect((await request(app).get("/messages/u1").set("x-test-user-id", "other")).status).toBe(403);
  });
  test("builds a conversation list from messages", async () => {
    ...
    const res = await request(app).get("/messages/u1").set("x-test-user-id", "u1");
  ...
  test("500 on failure", async () => {
    ...
    expect((await request(app).get("/messages/u1").set("x-test-user-id", "u1")).status).toBe(500);
  });
});

describe("createMessage", () => {
  test("400 without receiver or text", async () => {
    expect((await request(app).post("/messages").set("x-test-user-id", "c1").send({ text: "hi" })).status).toBe(400);
  });
  test("401 without a sender", async () => {
    expect((await request(app).post("/messages").send({ receiverId: "a1", text: "hi" })).status).toBe(401);
  });
  test("403 when the body senderId doesn't match auth", async () => {
    const res = await request(app).post("/messages").set("x-test-user-id", "c1").send({ receiverId: "a1", text: "hi", senderId: "evil" });
    expect(res.status).toBe(403);
  });
  test("403 when the pair hasn't accepted a request", async () => {
    const res = await request(app).post("/messages").set("x-test-user-id", "c1").send({ receiverId: "a1", text: "hi" });
    expect(res.status).toBe(403);
  ...
  test("sends a message once the request is accepted and emits realtime", async () => {
    ...
    const res = await request(app).post("/messages").set("x-test-user-id", "c1").send({ receiverId: "a1", text: "check https://x.com out" });
  ...
  test("500 on failure", async () => {
    ...
    expect((await request(app).post("/messages").set("x-test-user-id", "c1").send({ receiverId: "a1", text: "hi" })).status).toBe(500);
  });
});

describe("deleteConversationForUser", () => {
  test("400 without ids", async () => {
    expect((await request(app).post("/messages/delete").set("x-test-user-id", "u1").send({})).status).toBe(400);
  });
  test("403 on a user mismatch", async () => {
    const res = await request(app).post("/messages/delete").set("x-test-user-id", "other").send({ userId: "u1", participantId: "p1" });
    expect(res.status).toBe(403);
  });
  test("records the deletion and posts a contact-cut system message", async () => {
    ...
    const res = await request(app).post("/messages/delete").set("x-test-user-id", "u1").send({ userId: "u1", participantId: "p1" });
...
describe("createMessageRequest", () => {
  test("400 when fields are missing", async () => {
    expect((await request(app).post("/requests").send({ artistId: "a1", text: "hi" })).status).toBe(400);
  });
  test("409 when already accepted", async () => {
    mockMessage.exists.mockResolvedValueOnce(true);
    const res = await request(app).post("/requests").set("x-test-user-id", "c1").send({ artistId: "a1", text: "hi" });
  ...
  test("creates a pending request and notifies the artist", async () => {
    ...
    const res = await request(app).post("/requests").set("x-test-user-id", "c1").send({ artistId: "a1", text: "see https://ref.com" });
    expect(res.body.ok).toBe(true);
...
describe("request inbox + accept/decline", () => {
  test("lists incoming pending requests", async () => {
    ...
    const res = await request(app).get("/requests/incoming").set("x-test-user-id", "a1");
    expect(res.body.requests).toHaveLength(1);
  });
  test("404 when accepting a request that isn't yours", async () => {
    expect((await request(app).post("/requests/r1/accept").set("x-test-user-id", "a1")).status).toBe(404);
  });
  test("accepts a request and emits conversation:accepted", async () => {
    ...
    const res = await request(app).post("/requests/r1/accept").set("x-test-user-id", "a1");
    expect(res.body.ok).toBe(true);
  ...
  test("declines a request, sends the preset message, and reports decline count", async () => {
    ...
    const res = await request(app).post("/requests/r1/decline").set("x-test-user-id", "a1");
...
describe("getGateStatus", () => {
  ...
  test("500 on failure", async () => {
    ...
    expect((await request(app).get("/gate/a1").set("x-test-user-id", "c1")).status).toBe(500);
  });
});
...
describe("getUnreadState", () => {
  ...
  test("returns unread conversations and pending request ids", async () => {
    ...
    const res = await request(app).get("/unread").set("x-test-user-id", "a1");
...
describe("getNotifications", () => {
  ...
  test("aggregates unread messages, pending requests, and pending bookings", async () => {
    ...
    const res = await request(app).get("/notifications").set("x-test-user-id", "a1");
...
describe("markConversationRead", () => {
  test("400 without a viewer/other", async () => {
    expect((await request(app).post("/read").send({})).status).toBe(400);
  });
  test("403 on a user mismatch", async () => {
    const res = await request(app).post("/read").set("x-test-user-id", "a1").send({ userId: "other", participantId: "c1" });
    expect(res.status).toBe(403);
  });
  test("acks the pair and emits read receipts", async () => {
    const res = await request(app).post("/read").set("x-test-user-id", "a1").send({ participantId: "c1" });
    expect(res.body.ok).toBe(true);
  ...
  test("500 on failure", async () => {
    ...
    expect((await request(app).post("/read").set("x-test-user-id", "a1").send({ participantId: "c1" })).status).toBe(500);
  });
});
...
describe("catch -> 500 paths", () => {
  test("createMessageRequest", async () => {
    mockMessage.exists.mockRejectedValue(new Error("db"));
    expect((await request(app).post("/requests").set("x-test-user-id", "c1").send({ artistId: "a1", text: "hi" })).status).toBe(500);
  });
  test("listIncomingRequests", async () => {
    mockMessage.find.mockReturnValue(qReject(new Error("db")));
    expect((await request(app).get("/requests/incoming").set("x-test-user-id", "a1")).status).toBe(500);
  });
  test("acceptMessageRequest", async () => {
    mockMessage.findOne.mockReturnValue(qReject(new Error("db")));
    expect((await request(app).post("/requests/r1/accept").set("x-test-user-id", "a1")).status).toBe(500);
  });
  test("declineMessageRequest", async () => {
    mockMessage.findOne.mockReturnValue(qReject(new Error("db")));
    expect((await request(app).post("/requests/r1/decline").set("x-test-user-id", "a1")).status).toBe(500);
  });
  test("getUnreadState", async () => {
    mockMessage.find.mockReturnValue(qReject(new Error("db")));
    expect((await request(app).get("/unread").set("x-test-user-id", "a1")).status).toBe(500);
  });
  test("getNotifications", async () => {
    mockMessage.find.mockReturnValue(qReject(new Error("db")));
    expect((await request(app).get("/notifications").set("x-test-user-id", "a1")).status).toBe(500);
  });
