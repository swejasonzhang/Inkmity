...
describe("createStudio", () => {
  test("401 unauthenticated", async () => {
    expect((await request(server).post("/studios").send({ name: "Ink" })).status).toBe(401);
  });
  test("400 without a name", async () => {
    expect((await request(server).post("/studios").set("x-test-user-id", "u1").send({})).status).toBe(400);
  });
  test("creates a studio owned by the actor with a clamped commission", async () => {
    const res = await request(server).post("/studios").set("x-test-user-id", "u1").send({ name: "Ink Co", defaultCommissionPct: 5 });
  ...
  test("500 on failure", async () => {
    ...
    expect((await request(server).post("/studios").set("x-test-user-id", "u1").send({ name: "X" })).status).toBe(500);
  });
  test("retries the slug when the first candidate is taken", async () => {
    mockStudio.findOne
      .mockReturnValueOnce(q({ _id: "taken" }))
      .mockReturnValueOnce(q(null));
    const res = await request(server).post("/studios").set("x-test-user-id", "u1").send({ name: "Ink" });
    expect(res.status).toBe(201);
...
describe("getStudio", () => {
  test("404 when missing", async () => {
    ...
    expect((await request(server).get("/studios/s1")).status).toBe(404);
  });
  test("strips private payout fields", async () => {
    ...
    const res = await request(server).get("/studios/s1");
  ...
  test("400 on a bad id (CastError)", async () => {
    ...
    expect((await request(server).get("/studios/bad")).status).toBe(400);
  });
});

describe("getMyStudios", () => {
  test("401 unauthenticated", async () => {
    expect((await request(server).get("/me/studios")).status).toBe(401);
  });
  test("merges owned and member studios without duplicates", async () => {
    ...
    const res = await request(server).get("/me/studios").set("x-test-user-id", "u1");
...
describe("updateStudio", () => {
  test("403 for a non-admin", async () => {
    ...
    expect((await request(server).patch("/studios/s1").set("x-test-user-id", "u1").send({ name: "X" })).status).toBe(403);
  });
  test("owner can update editable fields + clamp commission", async () => {
    ...
    const res = await request(server).patch("/studios/s1").set("x-test-user-id", "u1").send({ city: "NYC", defaultCommissionPct: -1 });
...
describe("listMembers", () => {
  test("403 for a non-admin", async () => {
    ...
    expect((await request(server).get("/studios/s1/members").set("x-test-user-id", "u1")).status).toBe(403);
  });
  test("returns members with effective commission and artist profile", async () => {
    ...
    const res = await request(server).get("/studios/s1/members").set("x-test-user-id", "u1");
...
describe("inviteArtist", () => {
  test("resolves a handle to a clerkId then upserts an invite", async () => {
    mockStudio.findById.mockReturnValue(q(makeStudio({ ownerClerkId: "u1" })));
    const res = await request(server).post("/studios/s1/invite").set("x-test-user-id", "u1").send({ handle: "@art", commissionPct: 0.4 });
    expect(res.status).toBe(201);
  ...
  test("404 when the handle has no user", async () => {
    ...
    expect((await request(server).post("/studios/s1/invite").set("x-test-user-id", "u1").send({ handle: "ghost" })).status).toBe(404);
  });
  test("404 when the target isn't an artist", async () => {
    mockStudio.findById.mockReturnValue(q(makeStudio({ ownerClerkId: "u1" })));
    mockArtist.findOne.mockReturnValue(q(null));
    expect((await request(server).post("/studios/s1/invite").set("x-test-user-id", "u1").send({ artistClerkId: "x" })).status).toBe(404);
  });
  test("400 without artistClerkId or handle", async () => {
    mockStudio.findById.mockReturnValue(q(makeStudio({ ownerClerkId: "u1" })));
    expect((await request(server).post("/studios/s1/invite").set("x-test-user-id", "u1").send({})).status).toBe(400);
  });
});

describe("getMyMemberships", () => {
  test("401 unauthenticated", async () => {
    expect((await request(server).get("/me/memberships")).status).toBe(401);
  });
  test("maps each membership to its studio with effective commission", async () => {
    ...
    const res = await request(server).get("/me/memberships").set("x-test-user-id", "art1");
...
describe("error handling (catch -> 500)", () => {
  test("getStudio 500 on a non-cast error", async () => {
    ...
    expect((await request(server).get("/studios/s1")).status).toBe(500);
  });
  test("getMyStudios 500", async () => {
    mockStudio.find.mockReturnValue(qReject(new Error("db")));
    expect((await request(server).get("/me/studios").set("x-test-user-id", "u1")).status).toBe(500);
  });
  test("updateStudio 500", async () => {
    mockStudio.findById.mockReturnValue(qReject(new Error("db")));
    expect((await request(server).patch("/studios/s1").set("x-test-user-id", "u1").send({})).status).toBe(500);
  });
  test("listMembers 500", async () => {
    mockStudio.findById.mockReturnValue(qReject(new Error("db")));
    expect((await request(server).get("/studios/s1/members").set("x-test-user-id", "u1")).status).toBe(500);
  });
  test("inviteArtist 500", async () => {
    mockStudio.findById.mockReturnValue(qReject(new Error("db")));
    expect((await request(server).post("/studios/s1/invite").set("x-test-user-id", "u1").send({ artistClerkId: "x" })).status).toBe(500);
  });
  test("getMyMemberships 500", async () => {
    mockMembership.find.mockReturnValue(qReject(new Error("db")));
    expect((await request(server).get("/me/memberships").set("x-test-user-id", "art1")).status).toBe(500);
  });
  test("respondToInvite 500", async () => {
    mockMembership.findById.mockReturnValue(qReject(new Error("db")));
    expect((await request(server).post("/memberships/m1/respond").set("x-test-user-id", "art1").send({ action: "accept" })).status).toBe(500);
  });
  test("removeMember 500", async () => {
    mockStudio.findById.mockReturnValue(qReject(new Error("db")));
    expect((await request(server).delete("/studios/s1/members/art1").set("x-test-user-id", "art1")).status).toBe(500);
  });
  test("setStudioVerification 500", async () => {
    mockStudio.findById.mockReturnValue(qReject(new Error("db")));
    expect((await request(server).post("/studios/s1/verify").set("x-test-user-id", ADMIN_ID).send({ status: "verified" })).status).toBe(500);
  });
  test("createStudioConnect delegates to sendError", async () => {
    mockStudio.findById.mockReturnValue(qReject(new Error("db")));
    expect((await request(server).post("/studios/s1/connect").set("x-test-user-id", "u1")).status).toBe(500);
  });
  test("getStudioConnectStatus delegates to sendError", async () => {
    mockStudio.findById.mockReturnValue(qReject(new Error("db")));
    expect((await request(server).get("/studios/s1/connect/status").set("x-test-user-id", "u1")).status).toBe(500);
  });
});
...
describe("admin/owner guards (403)", () => {
  test("inviteArtist 403 for a non-admin", async () => {
    mockStudio.findById.mockReturnValue(q(makeStudio({ ownerClerkId: "x" })));
    expect((await request(server).post("/studios/s1/invite").set("x-test-user-id", "u1").send({ artistClerkId: "a" })).status).toBe(403);
  });
  test("updateMember 403 for a non-admin", async () => {
    mockStudio.findById.mockReturnValue(q(makeStudio({ ownerClerkId: "x" })));
    expect((await request(server).patch("/studios/s1/members/art1").set("x-test-user-id", "u1").send({})).status).toBe(403);
  });
  test("createStudioAccountLink 403 for a non-owner", async () => {
    mockStudio.findById.mockReturnValue(q(makeStudio({ ownerClerkId: "x" })));
    expect((await request(server).post("/studios/s1/connect/link").set("x-test-user-id", "u1")).status).toBe(403);
  });
  test("getStudioConnectStatus 403 for a non-admin", async () => {
    mockStudio.findById.mockReturnValue(q(makeStudio({ ownerClerkId: "x" })));
    expect((await request(server).get("/studios/s1/connect/status").set("x-test-user-id", "u1")).status).toBe(403);
  });
  test("updateMember 500 when the write fails", async () => {
    mockStudio.findById.mockReturnValue(q(makeStudio({ ownerClerkId: "u1" })));
    mockMembership.findOne.mockReturnValue(qReject(new Error("db")));
    expect((await request(server).patch("/studios/s1/members/art1").set("x-test-user-id", "u1").send({ role: "manager" })).status).toBe(500);
  });
  test("createStudioAccountLink delegates to sendError on failure", async () => {
    ...
    expect((await request(server).post("/studios/s1/connect/link").set("x-test-user-id", "u1")).status).toBe(500);
  });
});
...
describe("createStudioConnect existing", () => {
  test("returns the existing Connect account without creating one", async () => {
    ...
    const res = await request(server).post("/studios/s1/connect").set("x-test-user-id", "u1");
...
describe("respondToInvite", () => {
  test("400 for an invalid action", async () => {
    expect((await request(server).post("/memberships/m1/respond").set("x-test-user-id", "art1").send({ action: "x" })).status).toBe(400);
  });
  test("403 when not the invited artist", async () => {
    mockMembership.findById.mockReturnValue(q(makeMembership({ artistClerkId: "other" })));
    expect((await request(server).post("/memberships/m1/respond").set("x-test-user-id", "art1").send({ action: "accept" })).status).toBe(403);
  });
  test("400 when the invite isn't pending", async () => {
    ...
    expect((await request(server).post("/memberships/m1/respond").set("x-test-user-id", "art1").send({ action: "accept" })).status).toBe(400);
  });
  test("accepting activates the membership", async () => {
    ...
    await request(server).post("/memberships/m1/respond").set("x-test-user-id", "art1").send({ action: "accept" });
    expect(m.status).toBe("active");
  });
  test("declining marks it declined", async () => {
    const m = makeMembership({ artistClerkId: "art1", status: "invited" });
    mockMembership.findById.mockReturnValue(q(m));
    await request(server).post("/memberships/m1/respond").set("x-test-user-id", "art1").send({ action: "decline" });
...
describe("updateMember + removeMember", () => {
  test("admin updates a member's commission and role", async () => {
    ...
    await request(server).patch("/studios/s1/members/art1").set("x-test-user-id", "u1").send({ commissionPct: 0.3, role: "manager" });
  ...
  test("an artist can remove themselves", async () => {
    ...
    const m = makeMembership({ artistClerkId: "art1" });
    mockMembership.findOne.mockReturnValue(q(m));
    const res = await request(server).delete("/studios/s1/members/art1").set("x-test-user-id", "art1");
    expect(res.body.ok).toBe(true);
    expect(m.status).toBe("removed");
  });
  test("a non-admin non-self cannot remove a member", async () => {
    mockStudio.findById.mockReturnValue(q(makeStudio({ ownerClerkId: "x" })));
    expect((await request(server).delete("/studios/s1/members/art1").set("x-test-user-id", "stranger")).status).toBe(403);
  });
});
...
describe("studio Connect", () => {
  test("403 when not the owner", async () => {
    mockStudio.findById.mockReturnValue(q(makeStudio({ ownerClerkId: "x" })));
    expect((await request(server).post("/studios/s1/connect").set("x-test-user-id", "u1")).status).toBe(403);
  });
  ...
  test("account link creates the account first when missing", async () => {
    mockStudio.findById.mockReturnValue(q(makeStudio({ ownerClerkId: "u1" })));
    const res = await request(server).post("/studios/s1/connect/link").set("x-test-user-id", "u1");
...
describe("setStudioVerification (admin)", () => {
  test("403 for non-admins", async () => {
    expect((await request(server).post("/studios/s1/verify").set("x-test-user-id", "u1").send({ status: "verified" })).status).toBe(403);
  });
  test("400 for an invalid status", async () => {
    expect((await request(server).post("/studios/s1/verify").set("x-test-user-id", ADMIN_ID).send({ status: "weird" })).status).toBe(400);
  });
  test("an admin verifies a studio", async () => {
    const studio = makeStudio();
    mockStudio.findById.mockReturnValue(q(studio));
    const res = await request(server).post("/studios/s1/verify").set("x-test-user-id", ADMIN_ID).send({ status: "verified" });
    expect(res.status).toBe(200);
...
describe("getStudioConnectStatus", () => {
  test("not connected when there is no account", async () => {
    ...
    const res = await request(server).get("/studios/s1/connect/status").set("x-test-user-id", "u1");
