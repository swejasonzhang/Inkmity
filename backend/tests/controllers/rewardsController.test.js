...
describe("getMyCredits", () => {
  test("401 when unauthenticated", async () => {
    const res = await request(server).get("/rewards/credits");
    expect(res.status).toBe(401);
  });

  test("checks for a birthday credit then returns the available balance", async () => {
    const res = await request(server).get("/rewards/credits").set("x-test-user-id", "c1");
...
describe("grantCreditToClient", () => {
  test("401 when unauthenticated", async () => {
    const res = await request(server).post("/rewards/grant").send({});
    expect(res.status).toBe(401);
  });

  test("403 for a non-admin", async () => {
    const res = await request(server).post("/rewards/grant").set("x-test-user-id", "c1").send({});
    expect(res.status).toBe(403);
  });

  test("400 when clientId is missing", async () => {
    const res = await request(server).post("/rewards/grant").set("x-test-user-id", ADMIN_ID).send({});
...
describe("getMyRewards", () => {
  ...
  test("returns the rewards summary", async () => {
    const res = await request(server).get("/rewards").set("x-test-user-id", "c1");
