...
describe("join", () => {
  test("401 when unauthenticated", async () => {
    expect((await request(app).post("/waitlist").send({})).status).toBe(401);
  });
  test("creates a waitlist entry", async () => {
    const res = await request(app).post("/waitlist").set("x-test-user-id", "c1").send({ artistId: "a1" });
  ...
  test("delegates to sendError on failure", async () => {
    ...
    expect((await request(app).post("/waitlist").set("x-test-user-id", "c1").send({})).status).toBe(500);
  });
});

describe("leave", () => {
  test("401 when unauthenticated", async () => {
    expect((await request(app).delete("/waitlist/w1")).status).toBe(401);
  });
  test("404 when the entry isn't found", async () => {
    mockLeaveWaitlist.mockResolvedValue(null);
    expect((await request(app).delete("/waitlist/w1").set("x-test-user-id", "c1")).status).toBe(404);
  });
  test("leaves the waitlist", async () => {
    const res = await request(app).delete("/waitlist/w1").set("x-test-user-id", "c1");
    expect(res.body).toEqual({ ok: true });
    expect(mockLeaveWaitlist).toHaveBeenCalledWith("c1", "w1");
  });
  test("delegates to sendError on failure", async () => {
    mockLeaveWaitlist.mockRejectedValue(new Error("boom"));
    expect((await request(app).delete("/waitlist/w1").set("x-test-user-id", "c1")).status).toBe(500);
  });
});

describe("mine", () => {
  test("401 when unauthenticated", async () => {
    expect((await request(app).get("/waitlist/mine")).status).toBe(401);
  });
  test("returns the client's waitlist", async () => {
    const res = await request(app).get("/waitlist/mine").set("x-test-user-id", "c1");
    expect(res.body).toHaveLength(1);
  });
  test("delegates to sendError on failure", async () => {
    mockGetMyWaitlist.mockRejectedValue(new Error("boom"));
    expect((await request(app).get("/waitlist/mine").set("x-test-user-id", "c1")).status).toBe(500);
  });
});

describe("forArtist", () => {
  test("401 when unauthenticated", async () => {
    expect((await request(app).get("/waitlist/artist")).status).toBe(401);
  });
  test("returns the artist's waitlist", async () => {
    const res = await request(app).get("/waitlist/artist").set("x-test-user-id", "a1");
    expect(res.body).toHaveLength(1);
    expect(mockGetArtistWaitlist).toHaveBeenCalledWith("a1");
  });
  test("delegates to sendError on failure", async () => {
    mockGetArtistWaitlist.mockRejectedValue(new Error("boom"));
    expect((await request(app).get("/waitlist/artist").set("x-test-user-id", "a1")).status).toBe(500);
  });
});
