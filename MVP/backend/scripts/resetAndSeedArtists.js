...
(async () => {
  try {
    ...
    const insertedClients = await User.insertMany(clients, { ordered: true });
    const clientIds = insertedClients.map((c) => c._id);
    console.log(`👥 Seeded ${insertedClients.length} clients`);

    const artistsPayload = Array.from({ length: ARTIST_COUNT }).map((_, i) => {
    ...
    const insertedArtists = await User.insertMany(artistsPayload, {
      ordered: true,
    });
