...
function makeWeeklyDefault() {
  ...
  for (const k of WEEKDAY_KEYS) {
    weekly[k] = DEFAULT_OPEN_RANGES.map((r) => ({ ...r }));
  }
  return weekly;
}
...
export async function getAvailability(req, res) {
  ...
  const normalized = {
    ...
    slotMinutes: Math.max(
      5,
      Math.min(480, Number(doc.slotMinutes || DEFAULT_SLOT_MINUTES))
    ),
    weekly:
      doc.weekly && Object.keys(doc.weekly || {}).length
        ? doc.weekly
        : makeWeeklyDefault(),
