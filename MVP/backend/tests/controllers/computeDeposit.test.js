...
describe("computeDepositCents (deposit policy → deposit amount)", () => {
  ...
  test("percent: applies the minimum floor", () => {
    ...
    expect(computeDepositCents(p, 40000, "consultation")).toBe(5000);
  });
  ...
  test("percent: applies the maximum cap", () => {
    ...
    expect(computeDepositCents(p, 40000, "consultation")).toBe(10000);
  });
  ...
  test("missing/empty policy falls back to defaults without crashing", () => {
    expect(computeDepositCents({}, 40000, "consultation")).toBeGreaterThanOrEqual(0);
    expect(computeDepositCents(null, 40000, "consultation")).toBeGreaterThanOrEqual(0);
  });
});
