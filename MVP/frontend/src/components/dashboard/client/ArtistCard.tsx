...
const ArtistCard: React.FC<ArtistCardProps> = ({ artist, onClick }) => {
  ...
  const years =
    typeof artist.yearsExperience === "number" && artist.yearsExperience >= 0
    ...
    >
      <div className="relative w-full">
        ...
        >
          ...
          <div
            ...
            style={{ background: "linear-gradient(to top, color-mix(in oklab, var(--bg) 90%, transparent), transparent)" }}
          />
          <div className="absolute left-1/2 top-[46%] sm:top-1/2 -translate-x-1/2 -translate-y-[60%] sm:-translate-y-1/2 grid place-items-center gap-2">
            <div
              className="relative rounded-full overflow-hidden h-36 w-36 sm:h-40 sm:w-40 md:h-44 md:w-44 shadow-2xl ring-2 ring-[color:var(--card)]"
            ...
            >
              {avatarOk && artist.profileImage ? (
                ...
                <span
                  className="absolute inset-0 grid place-items-center text-3xl sm:text-4xl font-semibold"
                  style={{ color: "var(--fg)" }}
                >
                  {initials}
                </span>
      ...
      <div className="px-7 pt-6 pb-8 flex-1 min-h-0 flex flex-col gap-7">
        <div className="flex flex-col items-center text-center gap-4 sm:gap-5">
          ...
          <p
            className="text-sm leading-relaxed max-w-prose mt-1.5 md:mt-2 md:text-base"
