...
import { DayPicker, DayButton as RDPDayButton, getDefaultClassNames } from "react-day-picker";
...
const Chevron = ({ className, orientation, ...p }: any) =>
  orientation === "left" ? (
...
function CalendarDayButton({ className, day, modifiers, ...props }: React.ComponentProps<typeof RDPDayButton>) {
  ...
  return (
    <Button
      ...
      className={cn(
        "no-theme-anim flex aspect-square size-auto w-full min-w-(--cell-size) flex-col gap-1 leading-none font-normal [&>span]:text-xs [&>span]:opacity-70",
...
const MEMO_COMPONENTS = {
  Root: ({ className, rootRef, ...p }: any) => <div data-slot="calendar" ref={rootRef} className={cn("no-theme-anim", className)} {...p} />,
  Chevron,
  DayButton: CalendarDayButton,
  WeekNumber: ({ children, ...p }: any) => (
    <td {...p}>
...
function CalendarBase({
  ...
  const memoClassNames = React.useMemo(
    () => ({
      ...
      week: cn("flex w-full mt-2 sm:gap-x-1.5 md:gap-x-2", defaultClassNames.week),
  ...
  const memoComponents = React.useMemo(() => ({ ...MEMO_COMPONENTS, ...components }), [components]);

  const memoFormatters = React.useMemo(() => ({ ...STATIC_FORMATTERS, ...formatters }), [formatters]);
  ...
  return (
    <DayPicker
      ...
      formatters={memoFormatters}
      classNames={memoClassNames}
