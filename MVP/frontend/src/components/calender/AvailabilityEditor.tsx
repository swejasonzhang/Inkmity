      ...
      <div className="rounded-2xl border border-white/15 bg-gray-900/60 backdrop-blur p-6">
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-lg border ${
                activeDay === d.key
                  ...
                  : "bg-gray-900 text-white border-white/15 hover:bg-gray-800"
              }`}
            >
              {d.label}
            </button>
        ...
        <div className="mt-4 flex flex-col sm:flex-row sm:items-center gap-3">
          ...
          <div className="sm:ml-2">
            <button
              ...
              className="px-4 py-2 rounded-lg border border-white bg-white text-black hover:bg-gray-200 transition"
            >
              Add range
            </button>
          </div>
        ...
        <ul className="mt-4 space-y-2">
          {(weekly[activeDay] || []).map((r, i) => (
            <li
              key={i}
              className="flex items-center justify-between bg-gray-900 border border-white/10 rounded-lg px-3 py-2"
            >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="rounded-2xl border border-white/15 bg-gray-900/60 backdrop-blur p-6">
          ...
          {Object.keys(exceptions).length === 0 && (
            <p className="mt-4 text-sm text-white/60">No exceptions added.</p>
          )}
        </div>
              ...
              <div key={dateKey} className="rounded-lg border border-white/10">
                ...
                <ul className="p-2 space-y-2">
                  {(ranges || []).map((r, i) => (
                    <li
                      key={i}
                      className="flex items-center justify-between bg-gray-900 border border-white/10 rounded-md px-3 py-2"
                    >
                      <span>
                        {r.start} – {r.end}
                      </span>
