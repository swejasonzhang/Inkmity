...
const Header: React.FC<HeaderProps> = ({
    ...
    ? createPortal(
      <div className="md:hidden fixed inset-0 z-[2147483647]">
        ...
        <div className="absolute inset-0 bg-app flex flex-col text-app [&_*]:text-app [&_*]:border-app">
          <div className={`flex items-center justify-between px-5 border-b border-app ${MOBILE_HEADER_H}`}>
            <div className="flex items-center gap-3">
              <img src={logoSrc} alt="Inkmity Logo" className={`${MOBILE_LOGO_H} w-auto object-contain`} />
            </div>
            <Button
              aria-label="Close menu"
              variant="ghost"
              className="p-4 rounded-lg hover:bg-elevated active:scale-[0.98] text-app"
  ...
  return (
    <>
      ...
      <header
        className={[
          "md:hidden w-full flex items-center z-50",
          "px-5",
          `border-b ${MOBILE_HEADER_H}`,
          "text-app",
          isAuthPage ? "bg-transparent border-transparent" : "bg-app border-app",
        ].join(" ")}
      >
        <Link to={homeHref} className="flex items-center gap-3">
          <img src={logoSrc} alt="Inkmity Logo" className={`${MOBILE_LOGO_H} w-auto object-contain`} draggable={false} />
          <span className="sr-only">Inkmity</span>
        </Link>
